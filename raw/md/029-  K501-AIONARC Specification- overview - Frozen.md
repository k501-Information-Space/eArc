...
This phase prepares the repository for the frozen K501-AIONARC specification without changing ontology or axioms. It aligns the codebase with a `src/` layout, removes drift, and makes later phases mechanically safer.[[docs.pytest](https://docs.pytest.org/en/stable/explanation/goodpractices.html)]
...
`src/ └── k501/     ├── __init__.py    ├── core/    ├── storage/    ├── graph/    └── evidence/ tests/ pyproject.toml pytest.ini ruff.toml mypy.ini`

---

# Phase_1_transition_kernel.md

This phase formalizes the transition kernel only. It introduces canonical serialization, transition identity, transition schema, and identity validation without touching storage runtime behavior.[[github](https://github.com/trailofbits/rfc8785.py)]
...
`src/k501/core/ ├── __init__.py ├── canonical.py ├── transition.py ├── projection.py ├── replay.py └── validator.py tests/unit/ ├── test_canonical.py ├── test_transition.py └── test_identity_validator.py`

---

# Phase_2_journal_runtime.md

This phase implements the authoritative append-only journal runtime. It must treat the journal as truth and make append failures explicit, with recoverability for tail corruption.[[web.stanford](https://web.stanford.edu/~ouster/cgi-bin/cs140-spring14/lecture.php?topic=recovery)]
...
`src/k501/storage/ ├── __init__.py ├── journal.py ├── index_store.py └── evidence_store.py tests/unit/ ├── test_journal_runtime.py └── test_index_store.py tests/integration/ └── test_journal_append_recover.py`

---

# Phase_3_segment_runtime.md

This phase adds optional segment handling without changing the journal’s authority. Segments are runtime accelerators and recovery units only.[[nebulasyncforge2](http://nebulasyncforge2.sbs/file-appender-a-complete-guide-to-appending-data-safely/)]
...
`src/k501/storage/ ├── __init__.py ├── journal.py ├── index_store.py ├── evidence_store.py └── segment_store.py tests/unit/ └── test_segment_store.py tests/integration/ └── test_segment_rebuild.py`

---

# Phase_4_archive_runtime.md

This phase introduces frozen archives as immutable snapshots of validated transition streams. Archives are not authority; they are sealed replay artifacts.[[news.ycombinator](https://news.ycombinator.com/item?id=21673372)]
...
`src/k501/storage/ ├── __init__.py ├── journal.py ├── segment_store.py ├── archive_store.py └── index_store.py tests/unit/ └── test_archive_store.py tests/integration/ └── test_archive_freeze_replay.py`

---

# Phase_5_replay_validator.md

This phase implements replay validation as a first-class deterministic check. It compares ordered transition IDs, derived projections, replay digest, and graph topology exactly as specified.[[datatracker.ietf](https://datatracker.ietf.org/doc/html/rfc8785)]
...
`src/k501/core/ ├── __init__.py ├── canonical.py ├── transition.py ├── projection.py ├── replay.py └── validator.py tests/unit/ └── test_replay_validator.py tests/integration/ └── test_replay_validation.py`

---

# Phase_6_graph_builder.md

This phase builds the transition-centric graph from transitions. The graph remains reconstructable and non-authoritative.[[docs.pytest](https://docs.pytest.org/en/stable/explanation/goodpractices.html)]
...
`src/k501/graph/ ├── __init__.py ├── models.py ├── store.py └── validator.py tests/unit/ └── test_graph_builder.py tests/integration/ └── test_graph_rebuild.py`

---

# Phase_7_cli.md

This phase adds the command-line surface over the frozen kernel. It exposes operational commands only and does not alter truth semantics.[[realpython](https://realpython.com/ref/best-practices/project-layout/)]
...
`src/k501/cli/ ├── __init__.py ├── main.py └── commands/ tests/unit/ └── test_cli.py tests/integration/ └── test_cli_replay.py`

---

# Phase_8_api.md

This phase adds the HTTP API surface over the frozen runtime. The API is operational only and must not redefine architecture.[[realpython](https://realpython.com/ref/best-practices/project-layout/)]
...
`src/k501/api/ ├── __init__.py ├── middleware/ └── routes/ tests/unit/ └── test_api.py tests/integration/ └── test_api_replay.py`

---

# Phase_9_bootframes.md

This phase formalizes bootframes, startup state handling, and crash recovery sequencing. It does not alter the kernel; it makes runtime entry deterministic.[[web.stanford](https://web.stanford.edu/~ouster/cgi-bin/cs140-spring14/lecture.php?topic=recovery)]
...
`src/k501/core/ ├── __init__.py ├── canonical.py ├── transition.py ├── projection.py ├── replay.py ├── validator.py ├── lifecycle.py └── kernel.py tests/unit/ └── test_bootframes.py tests/integration/ └── test_boot_and_recover.py`
...
1. `Phase_0_repository_cleanup.md`
    
2. `Phase_1_transition_kernel.md`
    
3. `Phase_2_journal_runtime.md`
    
4. `Phase_3_segment_runtime.md`
    
5. `Phase_4_archive_runtime.md`
    
6. `Phase_5_replay_validator.md`
    
7. `Phase_6_graph_builder.md`
    
8. `Phase_7_cli.md`
    
9. `Phase_8_api.md`
    
10. `Phase_9_bootframes.md`
    

This order preserves the canonical direction of the specification: kernel first, storage next, replay and graph after that, then interfaces, then boot orchestration.[[datatracker.ietf](https://datatracker.ietf.org/doc/html/rfc8785)]

If you want, I can convert this into ten standalone markdown files exactly named as requested.